...
# ---------------------------------------------------------------------
# Utility: symmetric point overlap
# ---------------------------------------------------------------------
...
def evaluate_split(scene_id, split_id, split_dir, model, device, scene_out_base, debug=False):
    ...
    df: master GT dataframe
    scene_out_base: data/predictions/{BASE_CSV_NAME}&{scene_id}
    """
    ...
    # Filter directly using the loaded split_df from GroundTruth.csv
    if split_df.empty:
    ...
    world_points_np = world_points.squeeze(0).cpu().numpy()  # (N, H, W, 3)

    # Decode extrinsics for eps
    image_hw = world_points_np.shape[1:3]
    ...
    # Point clouds
    pointclouds = []
    for i in range(world_points_np.shape[0]):
    ...
    avg_baseline = dists[dists > 0].mean()
    eps = 0.05 * avg_baseline
    print(f"[INFO] {scene_id} split {split_id}: {len(pointclouds)} views, baseline={avg_baseline:.3f}, eps={eps:.4f}")
    ...
    split_metrics_path = os.path.join(split_out_dir, "metrics.csv")
    pd.DataFrame(
        [{
            "scene_id": scene_id,
            "split_id": split_id,
    ...
    ).to_csv(split_metrics_path, index=False)
...
def evaluate_scene(scene_dir, model, device="cpu"):
    ...
    scene_out_base = os.path.join(PRED_ROOT, f"{BASE_CSV_NAME}&{scene_id}")
...
def main():
    ...
    summary_path = os.path.join(PRED_ROOT, f"{BASE_CSV_NAME}_summary.csv")
