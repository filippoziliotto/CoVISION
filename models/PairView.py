...
class AttentiveLayerFusion(nn.Module):
    ...
    Output:
        logits (B,)
    """
    def __init__(self, emb_dim, hidden_dim=256, vec_gate=False, num_heads=4, dropout_p=0.2):
        super().__init__()
...
class EntropyAttentiveLayerFusion(nn.Module):
    """
    AttentiveLayerFusion head that also outputs per-layer gate distribution (alpha)
